...
| `list --tag <tag>` | **list** — タグフィルタ |
| `list --json` | **list** — JSON配列出力 |
| `add ...` | **add** — エントリ追加（--tag対応） |
| `memo ...` | **memo** — メモを更新（\nで改行） |
| `remove ...` | **remove** — エントリ削除 |
| `sync` | **sync** — 環境から自動検出して同期（差分表示） |
| `search <query>` | **search** — AND検索 |
| `tag <key> <tags>` | **tag** — タグを設定（上書き） |
| `export` | **export** — Markdownエクスポート |
| `export --format json` | **export** — JSONエクスポート |
...
- カタログファイルが存在しない場合は空のストアを作成: `{"version": 2, "entries": {}}`
- エントリのキーは `{type}:{name}` 形式（例: `skill:nano-banana`, `mcp:memory`）
- 既存エントリの上書き時は確認を求める
- v1形式のストアは自動的にv2に変換される（tags:[], description:''が補完される）
